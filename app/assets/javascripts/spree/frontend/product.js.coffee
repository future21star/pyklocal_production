$ ->
  Spree.addImageHandlers = ->
    ...
    thumbnails.find('a').on 'click', (event) ->
      ...
      ($ '#main-image img').attr 'data-zoom-image', ($ this).data('zoom-image')
      ($ this).mouseout ->
  ...
  Spree.showVariantImages = (variantId) ->
    ...
    if not currentThumb.hasClass('vtmb-' + variantId)
      ...
      ($ '#main-image img').attr 'data-zoom-image', thumb.find('a').data('zoom-image')
  ...
  selectBox = ($ '#variant_id')
  Spree.showVariantImages selectBox.val()
  Spree.updateVariantPrice selectBox
  selectBox.change (event) ->
    Spree.showVariantImages @value
    ($ '.additional-charge').addClass("hide")
    ($ '#variant_id_' + @value).removeClass("hide")
    Spree.updateVariantPrice ($ this)  